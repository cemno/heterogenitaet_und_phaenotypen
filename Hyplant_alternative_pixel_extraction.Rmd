...
The idea is to reduce the data in there dimensions and detect where the largest variation lies. By comparing the principal components of the FloX and the HyPlant we can detect whether both sensors captured the same variation and which approach is best suited for data extraction.
```{r}
pca_1229_minimalistic <- prcomp(spc_1229_minimalistic, center = TRUE)
pca_1229_normal <- prcomp(spc_1229_normal, center = TRUE)
pca_1242_minimalistic <- prcomp(spc_1242_minimalistic, center = TRUE)
pca_1242_normal <- prcomp(spc_1242_normal, center = TRUE)


#fviz_eig(pca_1229_minimalistic)
par(mar = c(4,6,1,0)+.1, mfrow = c(2,2), oma = c(1,1,4,1), yaxs = "r")
