...
frq_pixel_minimalistic_1229 <- mutate(filter(frq_pixel_minimalistic_1229, Plots == "3 x 6 m", PlotID %in% bands_mean_minimalistic_1229$PLOTID), measurement = "1229", approach =  "minimalistic")
frq_pixel_minimalistic_1242 <- mutate(filter(frq_pixel_minimalistic_1242, Plots == "3 x 6 m", PlotID %in% bands_mean_minimalistic_1229$PLOTID), measurement = "1242", approach =  "minimalistic")
frq_pixel_normal_1229 <- mutate(filter(frq_pixel_normal_1229, Plots == "3 x 6 m", PlotID %in% bands_mean_minimalistic_1229$PLOTID), measurement = "1229", approach =  "normal")
frq_pixel_normal_1242 <- mutate(filter(frq_pixel_normal_1242, Plots == "3 x 6 m", PlotID %in% bands_mean_minimalistic_1229$PLOTID), measurement = "1242", approach =  "normal")
...
ggplot(pixel_count_comparision, aes(x = Freq, fill = approach))+
  geom_bar() +
  ggtitle("Number of pixels per plot")+
  xlab("Pixel per Plot") +
  ylab("Counts")+
  facet_wrap(vars(measurement), labeller = labeller(measurement = measurement.labs), scales = "free_x", shrink = TRUE)+
  scale_fill_manual(values = rep(c("#fa3232", "#3296fa"), 2), name = "Extraction method:")+
  theme(panel.background = element_rect(colour = "#D9D9D9", size = 1),
...
# pca_1229 <- prcomp(pca_data_1229[,-c(1:4)], scale = TRUE)
# summary(pca_1229)
# 
# fviz_eig(pca_1229, title = "PCA: FloX and low resolution data")
# fviz_pca_ind(pca_1229,
#              habillage = pca_data_1229$approach,
#              col.ind = pca_data_1229$PlotID,
#              label = "ind.sup",
#              palette = "lanct",
#              geom.ind = "point",
#              addEllipses = TRUE,
...
# pca_1242 <- prcomp(pca_data_1242[,-c(1:4)], scale = TRUE)
# summary(pca_1242)
# 
# fviz_eig(pca_1242, title = "PCA: FloX and high resolution data")
# fviz_pca_ind(pca_1242,
#              habillage = pca_data_1242$approach,
#              col.ind = pca_data_1242$PlotID,
#              label = "ind.sup",
#              palette = "lanct",
#              geom.ind = "point",
#              addEllipses = TRUE,
...
pca_all <- prcomp(pca_data_all[,-c(1:4)], scale = TRUE)
summary(pca_all)

fviz_eig(pca_all, title = "PCA: all")
fviz_pca_ind(pca_all,
             habillage = pca_data_all$approach,
...
coor_all <- inner_join(coor_all, pca_data_all[,c(1:4)])
coor_all <- coor_all[,-1]
...
coor_1229_minimalistic_unsorted <- filter(coor_all, measurement == "1229" & approach == "minimalistic")
coor_1229_minimalistic <- coor_1229_minimalistic_unsorted[order(coor_1229_minimalistic_unsorted$PlotID),]

coor_1229_normal_unsorted <- filter(coor_all, measurement == "1229" & approach == "normal")
coor_1229_normal <- coor_1229_normal_unsorted[order(coor_1229_normal_unsorted$PlotID),]

coor_1242_minimalistic_unsorted <- filter(coor_all, measurement == "1242" & approach == "minimalistic")
coor_1242_minimalistic <- coor_1242_minimalistic_unsorted[order(coor_1242_minimalistic_unsorted$PlotID),]

coor_1242_normal_unsorted <- filter(coor_all, measurement == "1242" & approach == "normal")
coor_1242_normal <- coor_1242_normal_unsorted[order(coor_1242_normal_unsorted$PlotID),]

rm(coor_flox_unsorted, coor_1229_minimalistic_unsorted, coor_1229_normal_unsorted, coor_1242_minimalistic_unsorted, coor_1242_normal_unsorted)
...
distance_comparision <- rbind(distance_minimalistic_1229_df,
                              distance_normal_1229_df,
                              distance_minimalistic_1242_df, 
                              distance_normal_1242_df)

# # Calculate the distance between flox to minnimalistic/normal and flox to 1229/1242
# min_1229_vs_nom_1229 = abs(distance_minimalistic_1229 - distance_normal_1229)
# min_1242_vs_nom_1242 = abs(distance_minimalistic_1242 - distance_normal_1242)
# min_1229_vs_min_1242 = abs(distance_minimalistic_1229 - distance_minimalistic_1242)
# nom_1229_vs_nom_1242 = abs(distance_normal_1229 - distance_normal_1242)
...
```{r}
shapiro.test(distance_minimalistic_1229)
shapiro.test(distance_normal_1229)
shapiro.test(distance_minimalistic_1242)
shapiro.test(distance_normal_1242)
```
...
boxplot_comparision_approach <- ggplot(distance_comparision, aes(x = approach, y = distance, fill = approach, group = approach))+
  stat_boxplot(geom = "errorbar", size = 0.3, width = 0.1)+
  geom_boxplot(show.legend = FALSE, size = 0.3, position = "dodge")+
  ...
  geom_signif(comparisons = list(c("minimalistic","normal")), map_signif_level = TRUE, test = "t.test", test.args = list(alternative = "less", paired = TRUE), margin_top = 0.1, vjust = 0.4)+
...
approach.labs <- c("minimalistic extraction method", "normal extraction method")
names(approach.labs) <- c("minimalistic", "normal")

boxplot_comparision_resolution <- ggplot(distance_comparision, aes(x = resolution, y = distance, fill = resolution, group = resolution))+
  stat_boxplot(geom = "errorbar", size = 0.3, width = 0.1)+
  geom_boxplot(show.legend = FALSE, size = 0.3, position = "dodge")+
  ...
  geom_signif(comparisons = list(c("low","high")), map_signif_level = TRUE, test = "t.test", test.args = list(alternative = "two.sided", paired = TRUE), margin_top = 0.1, vjust = 0.1)+
  # For actual p.values instead of * use: map_signif_level = function(p) sprintf("p = %.2g", p)
  theme(panel.background = element_rect(fill = "#FFFFFF", colour = "#D9D9D9", size = 1),
        panel.grid = element_line(colour = "#D9D9D9"),
        plot.title = element_text(vjust = 2, hjust = 0.5),
...
```{r, fig.cap = "Comparison of the minimalistic and normal extraction methods by using the distance from FloX to each of the methods of the first two principal components. Separated between the low and high resolution data set. Paired one-sided students t-test, n = 15, p.value: * < 0.05, ** < 0.01, *** < 0.001 ."}
if(buffer == TRUE){
  boxplot_comparision_approach <- boxplot_comparision_approach + ggtitle("Distance between coordinates of FloX and each approach\nusing the buffered shapefiles")
  boxplot_comparision_resolution <- boxplot_comparision_resolution + ggtitle("Distance between coordinates of FloX and each resolution\nusing the buffered shapefiles")
  ggsave(plot = boxplot_comparision_approach, filename = "images/boxplot_comparision_approach_buffered.png")
  ggsave(plot = boxplot_comparision_resolution, filename = "images/boxplot_comparision_resolution_buffered´.png")
}else{
  boxplot_comparision_approach <- boxplot_comparision_approach + ggtitle("Distance between coordinates of FloX and each measurement")
  boxplot_comparision_resolution <- boxplot_comparision_resolution + ggtitle("Distance between coordinates of FloX and each resolution")
  ggsave(plot = boxplot_comparision_approach, filename = "images/boxplot_comparision_approach.png")
  ggsave(plot = boxplot_comparision_resolution, filename = "images/boxplot_comparision_resolution.png")
}
boxplot_comparision_approach
boxplot_comparision_resolution
```
