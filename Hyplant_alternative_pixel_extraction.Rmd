...
# )
```
###### Hyplant minimalistic methode, low resolution eigenvalues
```{r}
...
# )
```
###### Hyplant normal methode, low resolution eigenvalues
```{r}
...
# )
```
###### Hyplant minimalistic methode, high resolution eigenvalues
```{r}
...
# )
```
###### Hyplant normal methode, high resolution eigenvalues
```{r}
...
if (buffer == TRUE){
  write.csv(distance_comparision, file = "results/distance_comparison_buffered.csv", row.names = FALSE)
}else{
...
distance_comparision <- read.csv("results/distance_comparison.csv")
distance_comparision_buffered <- read.csv("results/distance_comparison_buffered.csv")
```
...
```{r, results='hide'}
  ...
  boxplot_comparison_approach <- ggplot(distance_comparision, aes(x = approach, y = distance, fill = approach, group = approach))+
    ...
    ggtitle("Score distance between FloX and each approach")+
  ...
  boxplot_comparison_resolution <- ggplot(distance_comparision, aes(x = resolution, y = distance, fill = resolution, group = resolution))+
    ...
    ylab("score distance")+
    ggtitle("Score distance between FloX and each resolution")+
...
```
```{r, fig.cap = "Comparison of the low and high resolution data sets by using the score distance from FloX to each of the resolutions with the first two principal components. Separated between the minimalistic and normal extraction method. Paired one-sided students t-test, n = 15, p.value: * < 0.05, ** < 0.01, *** < 0.001 ."}
...
```
###### Graphs {.active}
```{r, fig.cap = "Comparison of the minimalistic and normal extraction methods by using the score distance from FloX to each of the methods of the first two principal components. Separated between the low and high resolution data set using the buffered plots. Paired one-sided students t-test, n = 15, p.value: * < 0.05, ** < 0.01, *** < 0.001 ."}
boxplot_comparison_approach_buffered
```
```{r, fig.cap = "Comparison of the low and high resolution data sets by using the score distance from FloX to each of the resolutions with the first two principal components. Separated between the minimalistic and normal extraction method using the buffered plots. Paired one-sided students t-test, n = 15, p.value: * < 0.05, ** < 0.01, *** < 0.001 ."}
