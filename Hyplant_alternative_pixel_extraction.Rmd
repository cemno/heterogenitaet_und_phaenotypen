...
knitr::kable(rename(summarise(group_by(frq_pixel, Freq, Plots), "Occurrences" = n()), "Pixel per plot" = Freq), align = 'c')
```
##### Plot {.active}
```{r echo=FALSE, results='markup'}
...
if(!is_empty(diff)) print(paste0("WARNING: No pixel extracted for the following plots: ", diff))
```
...
tmp <- filter(select(FloX, datetime, Plot), Plot < 400 & Plot != "Soil")
tmp <- tmp[!duplicated(tmp$Plot),]
```
